...
describe('accessLogs(ctx, next)', () => {
  ...
  it('should attach stack trace if available', async () => {
    ...
    assert.ok(next.called)
    assert.deepEqual(logger.log.args[0][1].error, 'Error: Hello there')
    assert.deepEqual(logger.log.args[0][1].stack[0], 'Error: Hello there')
    assert.lengthOf(logger.log.args[0][1].stack, 13)
  })
